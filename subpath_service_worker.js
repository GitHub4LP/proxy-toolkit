...
self.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'FORCE_NAVIGATE_ALL_CLIENTS') {
        console.log('[Subpath SW] 收到强制刷新指令');
        ...
        self.clients.matchAll({
            ...
            console.log(`[Subpath SW] 强制刷新 ${clients.length} 个客户端`);
            
            // 立即触发所有导航，不等待完成
            clients.forEach(client => {
                client.navigate(client.url).catch(error => {
                    console.warn(`[Subpath SW] 导航失败: ${client.url}`, error);
                });
            });
        }).catch(error => {
            console.warn('[Subpath SW] 获取客户端失败:', error);
        });
    }
});
